...
name: prompt
description: >-
  Turn a rough request into a precise, fact-grounded spec. Use when the user says "write a spec", "refine this request", "sharpen this prompt", or "/prompt".
---
...
done-when a test can check. If the user's request is empty, ask for the request and stop.

## Workflow

### Phase 1 — Ground

- Run `.agents/tools/digest.sh` for the real toolchain, codemap, untested list, and
...
Emit the refined prompt, ready to hand to `the builder persona`:
...
- **Output contract** — what the builder returns (`GEMINI.md section: prompt-contract`).
