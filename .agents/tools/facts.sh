...
# how to build/test/lint every session. Caches to .agents/cache/facts.md.
#
...
build_full() {
  ...
  echo
  echo "## Quality gates (run them: \`.agents/tools/quality.sh\`)"
  _quality_tools
  echo
  echo "## Test framework (detected — write tests in this one)"
  echo "- $(_test_frameworks)"
  echo
  echo "_Generated by \`.agents/tools/facts.sh\`. Re-run with \`--refresh\` after toolchain changes._"
}
